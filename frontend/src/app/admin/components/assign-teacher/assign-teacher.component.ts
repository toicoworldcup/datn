...
export class AssignTeacherComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.teacherService
      .getAllTeachers()
      .subscribe((data) => (this.teacherList = data.map((t) => t.maGv)));
    this.classService
      .getAllClazzes()
      .subscribe((data) => (this.classList = data.map((c) => c.maLop)));
  ...
  assignTeacher(): void {
    ...
    this.assignmentService
      .assignTeacherClazz(this.assignMaLop, this.assignHocKi, this.assignMaGv)
      .subscribe({
        next: () => {
